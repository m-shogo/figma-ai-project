...
fixed_page_id="$(docker compose run --rm cli post create \
  ...
  --post_content='<h2 class="wp-block-heading">大見出し</h2><p>本文ギャップ確認</p><h3 class="wp-block-heading">中見出し</h3><p>本文ギャップ確認</p><h4 class="wp-block-heading">小見出し</h4><p>本文ギャップ確認</p><ul class="wp-block-list"><li>番号なしリスト</li></ul>' \
  --porcelain)"
[[ "$fixed_page_id" =~ ^[0-9]+$ ]] || {
