...
function fixture_image_id(?string $seed): int
{
    ...
    $upload = wp_upload_bits('standalone-lp-' . sanitize_file_name($seed) . '.png', null, $bytes);
    ...
    $attachment_id = wp_insert_attachment([
        ...
        'post_title' => 'Standalone LP fixture ' . $seed,
        'post_status' => 'inherit',
    ], $upload['file']);
...
$slug = sanitize_title((string) ($page_data['slug'] ?? 'standalone-lp'));
$title = sanitize_text_field((string) ($page_data['title'] ?? 'Standalone LP Fixture'));
