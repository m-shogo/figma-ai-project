...
`position: absolute` は**ファイル全体で2箇所だけ**です。
どちらも「箱の外へはみ出すあしらい」で、レイアウトには使っていません。

| 使っている場所 | 理由 |
| --- | --- |
| `.p-voice__balloon::before/after` | 吹き出しの尻尾。箱の外側へはみ出す三角 |
| `.p-footer__pagetop` | フッターの角に貼り付くボタン。本文の並びに影響させたくない |

重ねたい・ずらしたいときは、代わりにこの3つを使ってください。

| やりたいこと | 使うもの | 例 |
| --- | --- | --- |
| 要素どうしを重ねる | **grid の同じマス**（`grid-area: 1 / 1`） | メインビジュアルの写真とコピー / OPEN CAMPUS バッジ / 「まずは大学を体験してみよう！」 |
| 境目にまたがせる | **マイナスマージン** | `p-reason__title` / `p-courses__rec-label` / OPEN CAMPUS の吹き出しと丸 |
| 端に寄せる | **`justify-self` / `align-self`** | 各セクションの寄せ |

**hover も absolute を使いません。**

ボタンは**色が反転**します（白地 ⇄ 塗り）。アイコンも白版／色版へ差し替えます。
...
色の指定は `--btn-color` 1つにまとめてあるので、
`.c-btn--doc` / `.c-btn--oc` の色を変えれば枠線・文字・反転後の背景が
まとめて追従します。

丸いリンクは影に沈み込む動き（`translate` + `box-shadow`）です。
キーボード操作でも同じ見た目になるよう、すべて `:hover` と `:focus-visible` を
セットで書いています。
