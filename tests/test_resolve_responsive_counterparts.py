...
class ResponsiveCounterpartResolverTests(unittest.TestCase):
    ...
    def test_generic_sp_name_can_still_match_semantically(self) -> None:
        ...
        self.assertTrue(any(item.startswith("semantic_tokens=") for item in parts["evidence"]))
    ...
    def test_sparse_top_fingerprint_does_not_invent_a_high_match(self) -> None:
        result = resolve(load_fixture())
        top = by_pc(result, "839:4676")
        self.assertNotEqual(top["confidence"], "HIGH")
        self.assertNotEqual(top["decision"], "AUTO_CANDIDATE")
    ...
    def test_duplicate_generic_candidates_require_review(self) -> None:
        data = {
            "reference_id": "REF-X",
            "frames": [
                {
