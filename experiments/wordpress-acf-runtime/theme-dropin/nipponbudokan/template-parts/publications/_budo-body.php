<?php
/**
 * 号詳細の ACF 本文。空欄は出さない。投稿本文は出さない。
 *
 * Figma の「今月のおすすめ」は ACF `budo_pickup`、
 * 「今月のピックアップ」は ACF `monthpickup`。
 */
...
<?php if (nbk_acf_value_present($pickup)) : ?>
    <h3 class="wp-block-heading">今月のおすすめ</h3>
    <?php foreach ($pickup as $row) : ?>
        ...
        <?php if (nbk_acf_value_present($row_title)) : ?>
            <h4 class="wp-block-heading"><?php echo esc_html(wp_strip_all_tags((string) $row_title)); ?></h4>
        <?php endif; ?>
...
<?php if (nbk_acf_value_present($recommends)) : ?>
    <h3 class="wp-block-heading">今月のピックアップ</h3>
...
<?php foreach ($text_sections as $field => $label) : ?>
    ...
    <?php if (!nbk_acf_value_present($value)) : ?>
        <?php continue; ?>
    <?php endif; ?>
    <h3 class="wp-block-heading"><?php echo esc_html($label); ?></h3>
