...
if ! docker compose run --rm cli core is-installed >/dev/null 2>&1; then
  docker compose run --rm cli core install \
    --url="$WP_URL" \
    --title="WordPress ACF Runtime Smoke" \
...
http_code="$(curl --silent --show-error --output /tmp/acf-runtime-smoke.html --write-out '%{http_code}' "$WP_URL/")"
[[ "$http_code" == "200" ]] || {
...
if [[ "$THEME_IS_SAMPLE" == "1" ]]; then
  grep -q 'Sample Theme Fixture' /tmp/acf-runtime-smoke.html || {
    echo "FAIL fallback theme marker missing from rendered HTML." >&2
    exit 1
  }
  grep -Eiq 'name="robots"|name='"'"'robots'"'"'' /tmp/acf-runtime-smoke.html || {
    echo "FAIL rendered HTML must contain a robots meta tag." >&2
    exit 1
  }
  grep -Eiq 'noindex' /tmp/acf-runtime-smoke.html || {
...
echo "PASS WP-CLI core install + theme activation (${THEME_SLUG})"
...
if [[ "$THEME_IS_SAMPLE" == "1" ]]; then
  echo "PASS HTML noindex (sample theme)"
fi
