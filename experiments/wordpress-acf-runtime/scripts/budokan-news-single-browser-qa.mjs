...
async function measure(page) {
  return page.evaluate(() => {
    ...
    if (!pager || !back || !link || !span || !prev || !next) return null;
...
try {
  ...
  assert(sp, 'SP detail pager was not found.');
  ...
  assert(pc, 'PC detail pager was not found.');
  ...
  console.log('PASS Budokan News single SP return-to-list geometry and conditional-state QA.');
  console.log('PASS Budokan News single PC return-to-list geometry and conditional-state QA.');
} finally {
  await browser.close();
}
