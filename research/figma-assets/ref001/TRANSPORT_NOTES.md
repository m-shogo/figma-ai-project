...
The Drive/GAS/chat bridge sections above are retained as **historical incident documentation and fallback knowledge only**. They are no longer the repository-wide default production path for Figma raster promotion.

The current default is defined by `docs/figma-raster-export-policy.md`:
...
Do not read the historical `Drive / Git handoff` section as a current mandate. Use it only if a local filesystem-capable path is unavailable and the fallback route is explicitly chosen and validated under the same integrity / decode / mapping / visual gates.
