...
from evidence_visibility import classify_stylesheet_visibility, instrumentation_collision_report, measurement_target_report
...
class VisualCauseTests(unittest.TestCase):
    ...
    def test_cross_origin_stylesheet_visibility_caps_css_owner_confidence(self):
        result = classify_stylesheet_visibility([
            {"href": "https://cdn.example/library.css", "accessible": False, "error": "SecurityError"},
            {"href": "http://127.0.0.1:8772/styles.css", "accessible": True, "ruleCount": 86},
            {"href": None, "accessible": True, "ruleCount": 1},
        ])
        self.assertEqual("partial", result["kind"])
        self.assertEqual(1, result["blockedExternalStylesheets"])
        self.assertFalse(result["canClaimCompleteCssOwnership"])
        self.assertLess(result["cssOwnerConfidenceCeiling"], 1.0)

    def test_project_instrumentation_namespace_avoids_third_party_attribute_collision(self):
        collision = instrumentation_collision_report(
            ["data-calendar-view", "data-qa-state"],
            ["data-calendar-view", "data-date"],
        )
        namespaced = instrumentation_collision_report(
            ["data-ref002-calendar-view", "data-ref002-event"],
            ["data-calendar-view", "data-date"],
        )
        self.assertTrue(collision["collision"])
        self.assertEqual(["data-calendar-view"], collision["collisions"])
        self.assertFalse(namespaced["collision"])

    def test_hidden_selected_measurement_target_is_not_valid_geometry(self):
        result = measurement_target_report([
            {"selector": ".more--pc", "selected": True, "display": "none", "visibility": "visible", "width": 0, "height": 0},
            {"selector": ".more--sp", "display": "flex", "visibility": "visible", "width": 150, "height": 36},
        ])
        self.assertEqual("hidden-selected-visible-alternative", result["kind"])
        self.assertFalse(result["canUseGeometry"])
        self.assertFalse(result["selectedVisible"])

    def test_multiple_visible_measurement_targets_require_disambiguation(self):
        result = measurement_target_report([
            {"selector": ".item", "selected": True, "display": "block", "visibility": "visible", "width": 100, "height": 30},
            {"selector": ".item", "display": "block", "visibility": "visible", "width": 100, "height": 30},
        ])
        self.assertEqual("ambiguous-multiple-visible", result["kind"])
        self.assertFalse(result["canUseGeometry"])
