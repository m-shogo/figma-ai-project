...
| parser/AST-aware duplicate/specificity lint | standard + policy |
...
| importable JSON requirement | implementation targets / ACF docs |
...
| responsive images/srcset/sizes candidate | standard + QA |
| picture for real art direction | standard |
...
| Human change cost | stress QA + QA |

## Learning / repair

| Topic | Canonical owner |
| --- | --- |
| Fix canonical owner instead of symptom | standard |
...
| one failure does not create permanent ban | standard + pattern library |
| evidence promotion | standard + existing evidence maturity |
...
8. このmapへcanonical ownerを追加する。
9. Detail docが増えたらQuick Contractの可読性を優先し、AI entrypointを肥大化させない。
